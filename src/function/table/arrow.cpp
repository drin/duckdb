...
static unique_ptr<ArrowType> GetArrowLogicalTypeNoDictionary(ArrowSchema &schema) {
  ...
  if      (format == "n") { return make_uniq<ArrowType>(LogicalType::SQLNULL);   }
  else if (format == "b") { return make_uniq<ArrowType>(LogicalType::BOOLEAN);   }
  else if (format == "c") { return make_uniq<ArrowType>(LogicalType::TINYINT);   }
  else if (format == "s") { return make_uniq<ArrowType>(LogicalType::SMALLINT);  }
  else if (format == "i") { return make_uniq<ArrowType>(LogicalType::INTEGER);   }
  else if (format == "l") { return make_uniq<ArrowType>(LogicalType::BIGINT);    }
  else if (format == "C") { return make_uniq<ArrowType>(LogicalType::UTINYINT);  }
  else if (format == "S") { return make_uniq<ArrowType>(LogicalType::USMALLINT); }
  else if (format == "I") { return make_uniq<ArrowType>(LogicalType::UINTEGER);  }
  else if (format == "L") { return make_uniq<ArrowType>(LogicalType::UBIGINT);   }
  else if (format == "f") { return make_uniq<ArrowType>(LogicalType::FLOAT);     }
  else if (format == "g") { return make_uniq<ArrowType>(LogicalType::DOUBLE);    }

  //! this can be either decimal128 or decimal 256 (e.g., d:38,0)
  else if (format[0] == 'd') {
  ...
  else if (format == "u")    { return make_uniq<ArrowType>(LogicalType::VARCHAR, ArrowVariableSizeType::NORMAL); }
  else if (format == "U")    { return make_uniq<ArrowType>(LogicalType::VARCHAR, ArrowVariableSizeType::SUPER_SIZE); }
  else if (format == "tsn:") { return make_uniq<ArrowType>(LogicalTypeId::TIMESTAMP_NS); }
  else if (format == "tsu:") { return make_uniq<ArrowType>(LogicalTypeId::TIMESTAMP); }
  else if (format == "tsm:") { return make_uniq<ArrowType>(LogicalTypeId::TIMESTAMP_MS); }
  else if (format == "tss:") { return make_uniq<ArrowType>(LogicalTypeId::TIMESTAMP_SEC); }
  else if (format == "tdD")  { return make_uniq<ArrowType>(LogicalType::DATE, ArrowDateTimeType::DAYS); }
  else if (format == "tdm")  { return make_uniq<ArrowType>(LogicalType::DATE, ArrowDateTimeType::MILLISECONDS); }
  else if (format == "tts")  { return make_uniq<ArrowType>(LogicalType::TIME, ArrowDateTimeType::SECONDS); }
  else if (format == "ttm")  { return make_uniq<ArrowType>(LogicalType::TIME, ArrowDateTimeType::MILLISECONDS); }
  else if (format == "ttu")  { return make_uniq<ArrowType>(LogicalType::TIME, ArrowDateTimeType::MICROSECONDS); }
  else if (format == "ttn")  { return make_uniq<ArrowType>(LogicalType::TIME, ArrowDateTimeType::NANOSECONDS); }
  else if (format == "tDs")  { return make_uniq<ArrowType>(LogicalType::INTERVAL, ArrowDateTimeType::SECONDS); }
  else if (format == "tDm")  { return make_uniq<ArrowType>(LogicalType::INTERVAL, ArrowDateTimeType::MILLISECONDS); }
  else if (format == "tDu")  { return make_uniq<ArrowType>(LogicalType::INTERVAL, ArrowDateTimeType::MICROSECONDS); }
  else if (format == "tDn")  { return make_uniq<ArrowType>(LogicalType::INTERVAL, ArrowDateTimeType::NANOSECONDS); }
  else if (format == "tiD")  { return make_uniq<ArrowType>(LogicalType::INTERVAL, ArrowDateTimeType::DAYS); }
  else if (format == "tiM")  { return make_uniq<ArrowType>(LogicalType::INTERVAL, ArrowDateTimeType::MONTHS); }
  else if (format == "tin")  { return make_uniq<ArrowType>(LogicalType::INTERVAL, ArrowDateTimeType::MONTH_DAY_NANO); }
...
unique_ptr<ArrowArrayStreamWrapper> ProduceArrowScan( const ArrowScanFunctionData& function
  ...
  auto& stream_schema = function.schema_root.arrow_schema;
  ...
  for (idx_t idx = 0; idx < column_ids.size(); idx++) {
    ...
    if (col_idx != COLUMN_IDENTIFIER_ROW_ID) {
      auto& col_schema = *(stream_schema.children[col_idx]);
      parameters.projected_columns.filter_to_col[idx]  = col_idx;
      parameters.projected_columns.projection_map[idx] = col_schema.name;
      parameters.projected_columns.columns.emplace_back(col_schema.name);
    }
  }
...
bool ArrowTableFunction::ArrowScanParallelStateNext( ClientContext&        context
                                                    ,const FunctionData*   bind_data_p
                                                    ,ArrowScanLocalState&  state
                                                    ,ArrowScanGlobalState& parallel_state) {
  // Grab mutex for the function to serialize access to ArrowScanGlobalState
  lock_guard<mutex> parallel_lock(parallel_state.main_mutex);

  // Check if done already
  if (parallel_state.done) { return false; }

  // Cache some things
  auto  batch_ndx = ++(parallel_state.batch_index);
  auto& arrow_stream   = parallel_state.stream;

  // Find the next non-empty chunk
  auto current_chunk = arrow_stream->GetNextChunk();
  while (current_chunk->arrow_array.release && current_chunk->arrow_array.length == 0) {
    current_chunk = arrow_stream->GetNextChunk();
  }

  // If the array has been released, we're done
  if (!current_chunk->arrow_array.release) {
    parallel_state.done = true;
    return false;
  }

  // Set local state from global state
  state.Reset();
  state.batch_index = batch_ndx;
...
ArrowTableFunction::ArrowScanInitGlobal( ClientContext &context
...
ArrowTableFunction::ArrowScanInitLocalInternal( ClientContext&            context
                                               ,TableFunctionInitInput&   input
...
unique_ptr<LocalTableFunctionState>
ArrowTableFunction::ArrowScanInitLocal( ExecutionContext&         context
                                       ,TableFunctionInitInput&   input
                                       ,GlobalTableFunctionState* global_state_p) {
