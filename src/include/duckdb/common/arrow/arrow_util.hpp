...
namespace duckdb {
  ...
  struct ArrowUtil {
    using ArrowTypeExtMap = unordered_map<idx_t, const shared_ptr<ArrowTypeExtensionData>>;
    static bool TryFetchChunk( ChunkScanState&  scan_state
                              ...
                              ,idx_t&           result_count
                              ,ErrorData&       error
	                            ,ArrowTypeExtMap  extension_type_cast);

    static idx_t FetchChunk( ChunkScanState&        scan_state
                            ,ClientProperties       options
                            ,idx_t                  chunk_size
                            ,ArrowArray*            out
	                          ,const ArrowTypeExtMap& extension_type_cast);
