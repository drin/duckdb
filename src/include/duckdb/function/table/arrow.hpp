...
namespace duckdb {
  ...
  struct ArrowScanFunctionData : public TableFunctionData {
    ...
    ArrowScanFunctionData( stream_factory_produce_t   scanner_producer_p
      ...
      :  lines_read(0)
        ,stream_factory_ptr(stream_factory_ptr_p)
        ,scanner_producer(scanner_producer_p) {}
  ...
  struct ArrowArrayScanState {
    ...
    ArrowRunEndEncodingState& RunEndEncoding() { return run_end_encoding; }
    ...
    void Reset() {
      ...
      for (auto &child : children) { child.second->Reset(); }
      owned_data.reset();
    }
  };
  ...
  struct ArrowScanLocalState : public LocalTableFunctionState {
    ...
    //! Type alias for templated unordered_map containing indexed "Scan states"
    using ScanStateMap = unordered_map<idx_t, unique_ptr<ArrowArrayScanState>>;

    unique_ptr<ArrowArrayStreamWrapper> stream;
    shared_ptr<ArrowArrayWrapper>       chunk;
    vector<column_t>                    column_ids;
    ScanStateMap                        array_states;
    ...
    explicit ArrowScanLocalState(unique_ptr<ArrowArrayWrapper> current_chunk)
      : chunk(current_chunk.release()) {}

    // functions
    void Reset() {
      chunk_offset = 0;
      for (auto &col : array_states) { col.second->Reset(); }
    }
  ...
  struct ArrowTableFunction {
    ...
    static unique_ptr<FunctionData> ArrowScanBind( ClientContext&          context
    ...
    static void ArrowToDuckDB( ArrowScanLocalState&      scan_state
                              ,const arrow_column_map_t& arrow_convert_data
                              ,DataChunk&                output
                              ,idx_t                     start
                              ,bool                      arrow_scan_is_projected = true);

    //! Get next scan state
    static bool ArrowScanParallelStateNext( ClientContext&        context
    ...
    ArrowScanInitGlobal(ClientContext &context, TableFunctionInitInput &input);

    //! Initialize Local State
    static unique_ptr<LocalTableFunctionState>
    ArrowScanInitLocalInternal( ClientContext&            context
                               ,TableFunctionInitInput&   input
                               ,GlobalTableFunctionState* global_state);

    static unique_ptr<LocalTableFunctionState>
    ArrowScanInitLocal( ExecutionContext&         context
                       ,TableFunctionInitInput&   input
                       ,GlobalTableFunctionState* global_state);
    ...
    static void PopulateArrowTableType( ArrowTableType&      arrow_table
  ...
  protected:
    ...
    static double ArrowProgress( ClientContext &context
